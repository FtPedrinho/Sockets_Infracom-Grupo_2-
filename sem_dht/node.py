...
class Node():
    ...
    def GET(self, hash_do_arquivo):
        #try:
            self.enviar(f'GET {hash_do_arquivo} {self.porta} {time.time()}', self.peers[1])  # Pede o arquivo, manda o hash do arquivo para o vizinho.
    ...
    #-----------------------------
        ...
        if tipo == 'GET':
            ...
            tempo = req.split(" ")[3]
            if self.porta != porta:
                if hash in self.files.keys():
                    self.send_file(porta, hash, tempo)  # Entra aqui caso ele contenha o arquivo, caso contrario, repassa o get
                else:
                    self.enviar(req,self.peers[1])
    ...
    def send_file(self, port, hashed, tempo):
        ...
        mClientSocket.close()
        print(f'O arquivo {self.files[hashed]} demorou {(time.time()-float(tempo))*1000} ms para ser recebido em busca sequencial\n')
